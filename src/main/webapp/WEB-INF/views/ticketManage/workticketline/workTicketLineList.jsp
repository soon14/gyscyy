			...
			<div class="col-lg-12 col-md-12 col-sm-12 search-content  padding-zero">
	            ...
	            <div class="clearfix">
	                <div class="form-group col-lg-3 col-md-3 col-sm-3 col-xs-12 padding-zero">
	                   ...
	                   <!--  <input id="searchequipmentName" class="inputWidth text-left" placeholder="请输入设备名称" type="text"> -->
	                </div>
	                <div class="form-group col-lg-3 col-md-3 col-sm-3 col-xs-12 padding-zero">
	            ...
	            <div class="clearfix">	
	                <div class="form-group col-lg-3 col-md-3 col-sm-3 col-xs-12 padding-zero">
	                   ...
	                   <!--  <input id="searchallowperson" class="inputWidth text-left" placeholder="请输入许可人" type="text"> -->
	                </div>
	                <div class="form-group col-lg-6 col-md-6 col-sm-6 col-xs-12  padding-zero">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
                       <h5 class="table-title header smaller blue" >电力线路第一种工作票列表</h5>
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker','selectbox'], function(A){
					...
					var exportExcel="";
					 workTicketDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/workTicketLine/searchData"),
							...
							columns: [
							          ...
							          {data: "identifyName",name:"identify",width: "6%",orderable: true}
										...
										fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/workTicketLine/exportExcel"),JSON.stringify(conditions));
											 }
											 exportExcel="";
										 },
							toolbars: [{
											...
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														...
														url : format_url("/workTicketLine/getAdd")
													});
            									} else{
...
// 										});
								...
								events:{
									click: function(event){
										...
										var url = format_url('/workTicketLine/bulkDelete/'+ids);
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketLine/updateValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														render : '#page-container',
														url:format_url("/workTicketLine/getEdit/"+ id),
													});
            									} else{
            										alert(result.errorMsg);
            									}
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketLine/deleteValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													var url =format_url('/workTicketLine/'+ id);
													A.confirm('您确认删除么？',function(){
														$.ajax({
															url : url,
			        										contentType : 'application/json',
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketLine/tijiaoValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													workticketDialog = new A.dialog({
														title:"提交确认",
														url:format_url("/workTicketLine/sureSubmit?id="+ id),
														height:450,
														width:500
													}).render();
            									} else{
						...
						/* {
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketLine/setValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						},  */
						{
								...
								events:{
									click: function(event, nRow, nData){
										...
										A.loadPage({
											render : '#page-container',
											url:format_url("/workTicketLine/detail/"+ id),
										});
									}
								}
						},
						{
							...
							events:{
								click: function(event, nRow, nData){
									var id = nData.id;
									$.ajax({
										url: format_url('/workTicketLine/invalidValidate/'+id),
										contentType : 'application/json',
										dataType : 'JSON',
										type: 'POST',
										data : JSON.stringify(nData),
										success: function(result){
											if(result.result == 'success'){
												workticketDialog = new A.dialog({
													title:"鉴定",
													url:format_url("/workTicketLine/invalid?id="+ id),
													height:350,
													width:600
												}).render();
        									} else{
					...
					}, 
					{
						...
						events:{
							click: function(event, nRow, nData){
								birtPrint("workTicketLine.rptdesign",nData.id);
							}
						}
					},{//zzq修改第二次需求
						id: "printAqcs",
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				...
				var url =format_url("/workLine/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

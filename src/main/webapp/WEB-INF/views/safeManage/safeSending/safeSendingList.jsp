			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >安全发文</h5>
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','my97datepicker'], function(A){
					...
					var trainPlanDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/safeSending/search"),
							...
							columns: [
							          ...
							          {data: "sendingName",width: "auto",orderable: true}, 
							          ...
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/safeSending/exportExcel"),JSON.stringify(conditions));
											 }
											 exportExcel="";
										 },
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                    						listFormDialog = new A.dialog({
                        						...
                        						height:460 ,
                        						title: "安全发文增加",
                        						url:format_url('/safeSending/getAdd'),
                        						closed: function(){
								...
								events:{
									click: function(event){
										...
										var url = format_url('/safeSending/bulkDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height:460 ,
											title: "安全发文修改",
											url:format_url('/safeSending/getEdit/' + id),
											closed: function(){
												trainPlanDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/safeSending/delete/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:460 ,
										title: "安全发文查看",
										url:format_url('/safeSending/getshowPage/' + id),
										closed: function(){
											trainPlanDatatables.draw(false);
										}
									}).render();

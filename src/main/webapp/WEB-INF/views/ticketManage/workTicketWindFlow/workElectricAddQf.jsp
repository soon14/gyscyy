			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeQf").on("click", function(){
						...
						url = format_url("/workTicketWindFlow/agreeQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormQf").submit();
    				});
					$("#disAgreeQf").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/workTicketWindFlow/disAgreeQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormQf").submit();
					});
				});
			});

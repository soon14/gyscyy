		...
		<script type="text/javascript">
		 ...
		 var scienceTechnologyPlanDatatables;
				...
				seajs.use(['combobox','combotree','my97datepicker','uploaddropzone'], function(A){
					...
					$('#addForm').validate({
						...
						submitHandler: function (form) {
							//添加按钮
							var url = format_url("/rollingMaintencePlan");
					...
					$('#technicalBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "生产技术处审核",
    						url:format_url("/rollingMaintencePlan/getAddSCJSC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#planBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处审核",
    						url:format_url("/rollingMaintencePlan/getAddJHJYC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#manageBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "单位领导审核",
    						url:format_url("/rollingMaintencePlan/getAddDWLDTL?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#excuteBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处下文执行",
    						url:format_url("/rollingMaintencePlan/getAddJHJYCXW?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#zfBtn').on('click',function(){
						...
						obj.id=${entity.id};
						$.ajax({
							url : format_url("/rollingMaintencePlan/disAgreeZf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+""),
							contentType : 'application/json',
							dataType : 'JSON',
							data : JSON.stringify(obj),
							cache: false,
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var taskId=$("#taskId").val();
				var procInstId=$("#procInstId").val();
				var url =format_url("/rollingMaintencePlan/againSubmit?workId="+id+"&selectUser="+selectUser+"&taskId="+taskId+"&procInstId="+procInstId);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

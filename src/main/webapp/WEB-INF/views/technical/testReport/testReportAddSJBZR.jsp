...
<html lang="zh">
	...
	<body>
		<div class="col-md-12" >
			...
			<input class="col-md-12" id="id" name="id" type="hidden"  maxlength="64" value="${testReportEntityId}">
    		...
    		<div style="margin-right:100px;">
    			<button id="disAgreeSJBZR" class="btn btn-xs btn-danger pull-right"  style="margin-right:15px;">
    				<i class="glyphicon glyphicon-remove-circle"></i>
    				驳回
    			</button>
    			<button id="disAgreeSJBZRUP" class="btn btn-xs btn-danger pull-right"  style="margin-right:15px;">
    				<i class="glyphicon glyphicon-remove-circle"></i>
    				驳回到上一级
    			</button>
    			<button id="agreeSJBZR" class="btn btn-xs btn-success pull-right"  style="margin-right:15px;">
			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeSJBZR").on("click", function(){
						...
						url = format_url("/testReport/agreeSJBZR?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormJxzg").submit();
    				});
					$("#disAgreeSJBZR").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/testReport/disAgreeSJBZR?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
					$("#disAgreeSJBZRUP").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/testReport/disAgreeSJBZRUP?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
				});
			});

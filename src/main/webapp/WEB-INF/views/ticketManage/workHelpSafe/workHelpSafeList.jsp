			...
			<div class="col-lg-12 col-md-12 col-sm-12 search-content padding-zero">
	            ...
	            <div class="clearfix">
	                ...
	                <div class="form-group col-lg-3 col-md-3 col-sm-3 col-xs-12 padding-zero">
	                    <label class="searchLabel" for="searchallowperson">执行人</label>：
	            ...
	            <div class="clearfix">
	                ...
	                <div class="form-group col-lg-6 col-md-6 col-sm-6 col-xs-12 padding-zero ">
	                    <label class="searchLabel longSearchLabel" for="form-field-1">执行时间</label>：
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
                       <h5 class="table-title header smaller blue" >继电保护安全措施票</h5>
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker','selectbox'], function(A){
					...
					var exportExcel="";
					 workTicketDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/workHelpSafe/searchData"),
							...
							columns: [
							          ...
							          {data: "code",width: "12%",orderable: true}, 
							          {data: "unitName",width: "16%",orderable: true},
							         // {data: "groupName",width: "6%",orderable: true}, 
							          {data: "guarderName",width: "13%",orderable: true}, 
							         // {data: "content",width: "10%",orderable: true}, 
							          {data: "equipmentName",width: "16%",orderable: true}, 
							          {data: "changeAllowName",width: "10%",orderable: true},
							          {data: "changeAllowDate",width: "10%",orderable: true}, 
							          {data: "workStatusName",name:"workStatus",width: "13%",orderable: true}, 
							...
							toolbars: [{
											...
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														...
														url : format_url("/workHelpSafe/getAdd")
													});
            									} else{
								...
								events:{
									click: function(event){
										...
										var url = format_url('/workHelpSafe/bulkDelete/'+ids);
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workHelpSafe/updateValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														render : '#page-container',
														url:format_url("/workHelpSafe/getEdit/"+ id),
													});
            									} else{
            										alert(result.errorMsg);
            									}
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workHelpSafe/deleteValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													var url =format_url('/workHelpSafe/'+ id);
													A.confirm('您确认删除么？',function(){
														$.ajax({
															url : url,
			        										contentType : 'application/json',
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workHelpSafe/tijiaoValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													workticketDialog = new A.dialog({
														title:"提交确认",
														url:format_url("/workHelpSafe/sureSubmit?id="+ id),
														height:400,
														width:500
													}).render();
            									} else{
						...
						/* {
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workHelpSafe/setValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						},  */
						{
								...
								events:{
									click: function(event, nRow, nData){
										...
										A.loadPage({
											render : '#page-container',
											url:format_url("/workHelpSafe/detail/"+ id),
										});
									}
								}
						}
...
// 									var id = nData.id;
// 									$.ajax({
// 										url: format_url('/workHelpSafe/invalidValidate/'+id),
// 										contentType : 'application/json',
// 										dataType : 'JSON',
// 										type: 'POST',
// 										data : JSON.stringify(nData),
...
// 												workticketDialog = new A.dialog({
// 													title:"鉴定",
// 													url:format_url("/workHelpSafe/invalid?id="+ id),
// 													height:350,
// 													width:600
// 												}).render();
//         									} else{
...
// 					}
						...
						events:{
							click: function(event, nRow, nData){
								birtPrint("work_helpSafe.rptdesign",nData.id);
							}
						}
					}
					]}

			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeSJB").on("click", function(){
						...
						url = format_url("/testReport/agreeSJB?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormJxzg").submit();
    				});
					$("#disAgreeSJB").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/testReport/disAgreeSJB?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
				});
			});

			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
                       <h5 class="table-title header smaller blue" >一级动火工作票列表</h5>
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker','selectbox'], function(A){
					...
					var searchEndplandateStartDiv = new A.my97datepicker({
						...
						options : {
								...
								skin : 'ext',
								maxDate: "",
								minDate: "#F{$dp.$D(\\'searchStartplandateStartDivId\\')}"
						}
					}).render();
					...
					var exportExcel="";
					 workTicketDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/workTicketFire/searchData"),
							...
							columns: [
							        ...
							        /*   {data: "isSetName",name:"isSet",width: "5%",orderable: true}, */
							          ...
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
// 												window.location.href=format_url("/workTicketFire/exportExcel?conditions="+JSON.stringify(conditions));
												 exportExcels(format_url("/workTicketFire/exportExcel"),JSON.stringify(conditions));
							...
							toolbars: [{
											...
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														...
														url : format_url("/workTicketFire/getAdd")
													});
            									 } else{
								...
								events:{
									click: function(event){
										...
										var url = format_url('/workTicketFire/bulkDelete/'+ids);
							...
							btns: [{
								...
								render: function(btnNode, data){
									if(data.workStatus!="1"&&data.workStatus!="15"){
										btnNode.hide();
									}
								},
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketFire/updateValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														render : '#page-container',
														url:format_url("/workTicketFire/getEdit/"+ id),
													});
            									} else{
            										alert(result.errorMsg);
            									}
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketFire/tijiaoValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						/* {
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketFire/setValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						},  */
						{
								...
								events:{
									click: function(event, nRow, nData){
										...
										A.loadPage({
											render : '#page-container',
											url:format_url("/workTicketFire/detail/"+ id),
										});
									}
								}
						},
						{
							...
							className: " edit",
							render: function(btnNode, data){
								if(data.workStatus!="16"&&data.workStatus!="15"){
									btnNode.hide();
								}
							},
							events:{
								click: function(event, nRow, nData){
									var id = nData.id;
									$.ajax({
										url: format_url('/workTicketFire/invalidValidate/'+id),
										contentType : 'application/json',
										dataType : 'JSON',
										type: 'POST',
										data : JSON.stringify(nData),
										success: function(result){
											if(result.result == 'success'){
												workticketDialog = new A.dialog({
													title:"鉴定",
													url:format_url("/workTicketFire/invalid?id="+ id),
													height:350,
													width:600
												}).render();
        									} else{
					...
					}, 
					{
						...
						events:{
							click: function(event, nRow, nData){
								birtPrint("workFire.rptdesign",nData.id);
							}
						}
					}
					]}
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				...
				var url =format_url("/workFire/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

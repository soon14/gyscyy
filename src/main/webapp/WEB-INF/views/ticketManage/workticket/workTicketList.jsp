			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
                       <h5 class="table-title header smaller blue" >电气第一种工作票列表</h5>
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker','selectbox'], function(A){
					...
					var searchEndplandateStartDiv = new A.my97datepicker({
						...
						options : {
								isShowWeek : false,
								skin : 'ext',
								maxDate: "#F{$dp.$D(\\'searchStartplandateEndDivId\\')}",
								minDate: ""
						}
					}).render();
					...
					var searchStartplandateEndDiv = new A.my97datepicker({
						...
						options : {
								isShowWeek : false,
								skin : 'ext',
								maxDate: "#F{$dp.$D(\\'searchEndplandateStartDivId\\')}",
								minDate: "",
						}
					}).render();
					...
					var exportExcel="";
					 workTicketDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/workTicket/searchData"),
											...
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														...
														url : format_url("/workTicket/getAdd")
													});
            									} else{
...
// 										});
								...
								events:{
									click: function(event){
										...
										var url = format_url('/workTicket/bulkDelete/'+ids);
							...
							btns: [{
									...
									click: function(event, nRow, nData){
										...
										var loginUser = '${userEntity.id}';
			 							if(workPerson!=loginUser){
			 								...
			 								var id = nData.id;
											$.ajax({
												url: format_url('/workTicket/updateValidate/'+id),
												contentType : 'application/json',
												dataType : 'JSON',
												type: 'POST',
												data : JSON.stringify(nData),
												success: function(result){
													if(result.result == 'success'){
														A.loadPage({
															render : '#page-container',
															url:format_url("/workTicket/getEdit/"+ id),
														});
	            									} else{
	            										alert(result.errorMsg);
	            									}
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicket/deleteValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													var url =format_url('/workTicket/'+ id);
													A.confirm('您确认删除么？',function(){
														$.ajax({
															url : url,
			        										contentType : 'application/json',
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicket/tijiaoValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													workticketDialog = new A.dialog({
														title:"提交确认",
														url:format_url("/workTicket/sureSubmit?id="+ id),
														height:450,
														width:500
													}).render();
            									} else{
						...
						/* {
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicket/setValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						},  */
						{
								...
								events:{
									click: function(event, nRow, nData){
										...
										A.loadPage({
											render : '#page-container',
											url:format_url("/workTicket/detail/"+ id),
										});
									}
								}
						}
									...
									$.ajax({
										...
										success: function(result){
											if(result.result == 'success'){
												workticketDialog = new A.dialog({
													title:"鉴定",
													url:format_url("/workTicket/invalid?id="+ id),
													height:350,
													width:600
												}).render();
        									} else{
						...
						events:{
							click: function(event, nRow, nData){
								birtPrint("workTicket.rptdesign",nData.id);
							}
						}
					},{//zzq修改第二次需求
						id: "printAqcs",

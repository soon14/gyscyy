package com.aptech.business.ticketManage.workTicketFire.web;
 ...
 */
@Controller
@RequestMapping("/workTicketFire")
public class WorkTicketFireController extends BaseController<WorkTicketFireEntity> {
	...
	@Override
	public IBaseEntityOperation<WorkTicketFireEntity> getService() {
		return workTicketFireService;
	}
...
//		List<SysUnitEntity> treeNodeList = sysUnitService.getUnitTreeNodeList();
        ...
        Map<String, SysDictionaryVO> codeDateTypeMap  =  DictionaryUtil.getDictionaries("WORKTICKETFIRE_TYPE");
		...
		return this.createModelAndView("ticketManage/workFire/workFireList", resultMap);
	}
    ...
    //添加的时候要给安全措施写一条无的记录，zzq修改结束 20180315
  	...
  	//添加的时候要给安全措施写一条无的记录，zzq修改开始 20180315
  		...
  		workSafeEntity=new WorkSafeEntity();
  		workSafeEntity.setSignerContent("无");
  		workSafeEntity.setSafeType(Long.valueOf(WorkSafeTypeEnum.SAFETWELVE.getCode()));
  		workSafeEntity.setOrderSeq(Long.valueOf(1));
  		workSafeEntity.setUuidCode(uuid);
  		workSafeService.addEntity(workSafeEntity);
  		//添加的时候要给安全措施写一条无的记录，zzq修改结束 20180315
  		String userUnitRels = "[]";
  		resultMap.put("userUnitRels", userUnitRels);
		return this.createModelAndView("ticketManage/workFire/workFireAdd", resultMap);
	}
	/**
	 *	跳转到修改页面
	 */
	...
	public ModelAndView getEditPage(HttpServletRequest request, @PathVariable Long id){
		Map<String, Object> resultMap = new HashMap<String, Object>();
		// 返回前台数据项
		WorkTicketFireEntity workTicketEntity=workTicketFireService.findById(id);
		...
		List<Condition> conditions=new ArrayList<Condition>();
  		...
  		WorkFireEntity workFireEntity=null;
  		List<WorkFireEntity> workElectricList=workFireService.findByCondition(conditions, null);
        if(!workElectricList.isEmpty()){
        	workFireEntity=workElectricList.get(0);
          	resultMap.put("workFireEntity", workFireEntity);
          }else{
        	workFireEntity=new WorkFireEntity();
          	resultMap.put("workFireEntity", workFireEntity);
        }
        //查询电气一结束
		...
		resultMap.put("userEntity", userEntity);
		//查询执行人下拉框
        ComboboxVO comboExecuteVO = new ComboboxVO();
        conditions.clear();
		conditions.add(new Condition("a.C_STATUS", FieldTypeEnum.STRING, MatchTypeEnum.EQ, String.valueOf(DataStatusEnum.NORMAL.ordinal())));
		conditions.add(new Condition("a.C_UNIT_ID", FieldTypeEnum.LONG, MatchTypeEnum.EQ,userEntity.getUnitId()));
		List<SysUserEntity> allUsers=sysUserService.findByCondition(conditions, null);
        for(SysUserEntity sysUserEntity : allUsers){
            comboExecuteVO.addOption(sysUserEntity.getId().toString(), sysUserEntity.getName());
        }
        resultMap.put("executeCombobox", JsonUtil.toJson(comboExecuteVO.getOptions()));
      //获取关联设备
		...
		return this.createModelAndView("ticketManage/workFire/workFireEdit", resultMap);
	}
	...
	public ModelAndView detail(HttpServletRequest request, @PathVariable Long id) throws ParseException{
		...
		WorkTicketFireEntity  workTicketEntity=workTicketFireService.findById(Long.valueOf(id));
		SysUserEntity userEntity= RequestContext.get().getUser();
		...
		resultMap.put("rlId", "");
		return this.createModelAndView("ticketManage/workFire/workFireDetail", resultMap);
	}
	/**
	 *	详细页面   第一个tab   
	 * @throws ParseException 
	...
	public ModelAndView workPjxxDetail(HttpServletRequest request, @PathVariable Long id) throws ParseException{
        ...
        if(!workElectricList.isEmpty()){
        	workFireEntity=workElectricList.get(0);
//        	SysUserEntity sysUserEntity=sysUserService.findById(workFireEntity.getOtherExecutorId());
//        	workFireEntity.setOtherExecutor(sysUserEntity.getName());
//            if(workFireEntity.getOtherExecutorTwo()!=null&&(!"".equals(workFireEntity.getOtherExecutorTwo()))){
//        	SysUserEntity sysUserEntity1=sysUserService.findById(Long.parseLong(workFireEntity.getOtherExecutorTwo()));
//            workFireEntity.setOtherExecutorTwo(sysUserEntity1.getName());
//            }
//            if(workFireEntity.getOtherExecutorThree()!=null&&(!"".equals(workFireEntity.getOtherExecutorThree()))){
//            SysUserEntity sysUserEntity2=sysUserService.findById(Long.parseLong(workFireEntity.getOtherExecutorThree()));
//            workFireEntity.setOtherExecutorThree(sysUserEntity2.getName());
//            }
          	resultMap.put("workFireEntity", workFireEntity);
          }else{
        	workFireEntity=new WorkFireEntity();
       ...
       //查询电气一结束
		...
		return this.createModelAndView("ticketManage/workFire/workFireDetailPjxx", resultMap);
	}
	/**
	 *	详细页面   第二个tab   
	 * @throws ParseException 
	...
	public ModelAndView workCardDetail(HttpServletRequest request, @PathVariable Long id) throws ParseException{
        ...
        resultMap.put("cardSortListFour", dicVoFour);
		return this.createModelAndView("ticketManage/workFire/workTicketDetailCard", resultMap);
	}
	/**
	 *	电器第一种票的审批页面
	 * @throws ParseException 
	...
	public ModelAndView approve(HttpServletRequest request, @PathVariable Long id,@PathVariable String type) throws ParseException{
		...
		conditionsLc.add(new Condition("procDef.KEY_", FieldTypeEnum.STRING, MatchTypeEnum.EQ, ProcessMarkEnum.WORK_TICKET_FIRE_PROCESS_KEY.getName()));
		...
		// 返回前台数据项
		WorkTicketFireEntity workTicketEntity=workTicketFireService.findById(id);
		resultMap.put("workTicketFireEntity", workTicketEntity);
...
//        resultMap.put("cardSortListFour", dicVoFour);
		...
		return this.createModelAndView("ticketManage/workFire/workFireApprove", resultMap);
	}
	...
	public ModelAndView sureSubmit(HttpServletRequest request){
		...
		resultMap.put("id", id);
		conditions.add(new Condition("C_DEFINITION_KEY", FieldTypeEnum.STRING, MatchTypeEnum.EQ,ProcessMarkEnum.WORK_TICKET_FIRE_PROCESS_KEY.getName()));
		...
		return new ModelAndView("/ticketManage/workFire/sureSubmitPerson",resultMap);
	}
	/**
	 * @Description:   审批确认弹出框
	 * @author         zhangzq 
	...
	public ModelAndView approveSubmit(HttpServletRequest request){
		...
		List<SysUserEntity> userList= nodeConfigService.getFirstNodeTransactor(modelId,"",starter);
		resultMap.put("userList", userList);
		return new ModelAndView("/ticketManage/workFire/approveSubmitPerson",resultMap);
	}
	/**
	...
	public @ResponseBody ResultObj update(@RequestBody WorkTicketFireEntity t, HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.update(t,id);
	}
	/** 批量删除
	 * @author         zhangzq 
	 * @Date           2017年6月20日 下午3:34:59 
	...
	public @ResponseBody ResultObj bulkDelete(@RequestBody List<Integer> ids) {
		...
		workTicketFireService.deleteBulk(ids);
		return resultObj;
	}
	/**
	 * @Description:   鉴定跳转页面
	...
	public ModelAndView invalid(HttpServletRequest request){
		...
		return new ModelAndView("/ticketManage/workFire/workInvalid",resultMap);
	}
	/**
	 * @Description:   鉴定
	 * @author         zhangzq 
	...
	public @ResponseBody ResultObj saveInvalid(@RequestBody WorkTicketFireEntity workTicketEntity, HttpServletRequest request) {
		return workTicketFireService.saveInvalid(workTicketEntity);
	}
	/**
	...
	public @ResponseBody ResultObj isExecute(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isExecute(id);
	}
	/**
	 * @Description:   申请试运
	 * @author         zhangzq 
	...
	public @ResponseBody ResultObj isExecuteSqsy(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isExecuteSqsy(id);
	}
	/**
	 * @Description:   试运恢复
	 * @author         zhangzq 
	...
	public @ResponseBody ResultObj isExecuteSyhf(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isExecuteSyhf(id);
	}
	/**
	 * @Description:   修改时候的验证
	 * @author         zhangzq 
	...
	public @ResponseBody ResultObj updateValidate(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.updateValidate(id);
	}
	...
	public @ResponseBody ResultObj deleteValidate(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.deleteValidate(id);
	}
	...
	public @ResponseBody ResultObj tijiaoValidate(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.tijiaoValidate(id);
	}
	/**
	...
	public @ResponseBody ResultObj invalidValidate(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.invalidValidate(id);
	}
	/**
	...
	public @ResponseBody ResultObj setValidate(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.setValidate(id);
	}
	/**
	 * @Description:   
	 * @author         zhangzq 
	...
	public void expData(HttpServletRequest req,HttpServletResponse res,Map<String, Object> params) throws UnsupportedEncodingException{
		...
		Page<WorkTicketFireEntity> page=new Page<WorkTicketFireEntity>();
		...
		List<WorkTicketFireEntity> dataList= new ArrayList<WorkTicketFireEntity>();
		for (String str : userNameList) {
			if (userEntity.getLoginName().equals(str)|| userEntity.getLoginName().equals("admin")||userEntity.getLoginName().equals("super")) {
				...
				conditions.add(new Condition("t.C_ISTYPICAL", FieldTypeEnum.STRING, MatchTypeEnum.EQ,Istypicaleum.ISSETYES.getCode()));
				conditions.add(new Condition("t.C_TYPE", FieldTypeEnum.STRING, MatchTypeEnum.EQ, WorkTypeEnum.FIREONE.getCode()));
				dataList = workTicketFireService.findByCondition(conditions, page);
			}else {
				dataList = workTicketFireService.findByCondition(conditions, page);
			}
		}
//		List<WorkTicketFireEntity> dataList=workTicketFireService.findByCondition(conditions, page);
		DateFormatUtil df= DateFormatUtil.getInstance("yyyy-MM-dd HH:mm");
		for (WorkTicketFireEntity workTicketEntity : dataList) {
			workTicketEntity.setPlandateStartString(df.format(workTicketEntity.getPlandateStart()));
			workTicketEntity.setPlandateEndString(df.format(workTicketEntity.getPlandateEnd()));
		}
		Map<String,Object> resultMap=new HashMap<String, Object>();
		resultMap.put("dataList", dataList);
		ExcelUtil.export(req, res, "一级动火工作票模板.xlsx","一级动火工作票.xlsx", resultMap);
	}
	...
	public ModelAndView tickeList(HttpServletRequest request, Map<String, Object> params) {
        ...
        resultMap.put("statusTypes", JsonUtil.toJson(codeDateTypesCombobox.getOptions()));
		return this.createModelAndView("ticketManage/workFire/tickeList", resultMap);
	}
	/**
	...
	public @ResponseBody ResultObj isQfAqcs(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isQfAqcs(id);
	}
	/**
	...
	public @ResponseBody ResultObj isQfAqcsAdd(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isQfAqcsAdd(id);
	}
	/**
	...
	public @ResponseBody ResultObj isXkAqcs(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isXkAqcs(id);
	}
	/**
	...
	public @ResponseBody ResultObj isXkAqcsAdd(HttpServletRequest request ,@PathVariable Long id) {
		return workTicketFireService.isXkAqcsAdd(id);
	}
	...
	public ModelAndView searchJan(HttpServletRequest request) throws Exception{
		...
		//zzq20180416修改查询班组列表开始
				...
				List<OrgaAppEntity> orgaApp = orgaAppService.findByCondition(conditionsBanzu, null);
				//zzq20180416修改查询班组列表结束
		ComboboxVO comboWorkTicketTwoVO = new ComboboxVO();
		for(OrgaAppEntity orgaAppEntity : orgaApp){
			comboWorkTicketTwoVO.addOption(orgaAppEntity.getId().toString(), orgaAppEntity.getName());
	    }
		//TODO下拉框具体内容根据具体业务定制
		resultMap.put("groupIdCombobox", JsonUtil.toJson(comboWorkTicketTwoVO.getOptions()));
      ...
      //工作负责人下拉列表
		...
		return this.createModelAndView("ticketManage/workFire/workTicketFireListJan", resultMap);
	...
	public @ResponseBody ResultListObj searchJanData(HttpServletRequest request, @RequestBody Map<String, Object> params) throws Exception {
		...
		String qualifiedCount = request.getParameter("qualifiedCount");
		String unQualifiedCount = request.getParameter("unQualifiedCount");
		Page<WorkTicketFireEntity> page =  PageUtil.getPage(params);
		  
			List<WorkTicketFireEntity> entities =workTicketFireService.searchJan(yearStart,yearEnd,params,page,identify,qualifiedCount,unQualifiedCount);
	...
	public ModelAndView runDetail(HttpServletRequest request) throws ParseException{
		...
		List<WorkTicketFireEntity> workTicketFireList = workTicketFireService.findByCondition(conditions, null);
		WorkTicketFireEntity workTicketFireEntity = workTicketFireList.get(0);
		resultMap.put("workTicketId", workTicketFireEntity.getId());
		SysUserEntity userEntity= RequestContext.get().getUser();
		
		conditions.clear();
		conditions.add(new Condition("sys_user.C_LOGIN_NAME", FieldTypeEnum.STRING, MatchTypeEnum.EQ, userEntity.getLoginName()));
		conditions.add(new Condition("procInst.BUSINESS_KEY_", FieldTypeEnum.STRING, MatchTypeEnum.EQ, workTicketFireEntity.getId()));
		conditions.add(new Condition("procDef.KEY_", FieldTypeEnum.STRING, MatchTypeEnum.EQ, ProcessMarkEnum.WORK_TICKET_PROCESS_KEY.getName()));
		conditions.add(new Condition("task.end_time_ IS NULL"));
		List<TodoTaskEntity> list =  todoTaskService.findByCondition(conditions, null);
		TodoTaskEntity todoTaskEntity=null;
		...
		resultMap.put("workStatus", workTicketFireEntity.getWorkStatus());
	...
	public @ResponseBody ResultObj add(@RequestBody WorkTicketFireEntity WorkTicketEntity, HttpServletRequest request) {
		workTicketFireService.addEntity(WorkTicketEntity);
		//保存三种人数据
	...
	public @ResponseBody  ResultListObj searchDate(HttpServletRequest request,@RequestBody Map<String, Object> params){
		...
		ResultListObj resultObj = new ResultListObj();
		SysUserEntity userEntity = RequestContext.get().getUser();
		List<WorkTicketFireEntity> windEntities = new ArrayList<WorkTicketFireEntity>();
		Page<WorkTicketFireEntity> page = PageUtil.getPage(params);
		...
		for (String str : userNameList) {
			if (userEntity.getLoginName().equals(str)|| userEntity.getLoginName().equals("admin")||userEntity.getLoginName().equals("super")) {
				...
				conditions.add(new Condition("t.C_ISTYPICAL", FieldTypeEnum.STRING, MatchTypeEnum.EQ,Istypicaleum.ISSETYES.getCode()));
				conditions.add(new Condition("t.C_TYPE", FieldTypeEnum.STRING, MatchTypeEnum.EQ, WorkTypeEnum.FIREONE.getCode()));
				windEntities = (List<WorkTicketFireEntity>)workTicketFireService.findByCondition(conditions, page);
			}else {
				windEntities =  (List<WorkTicketFireEntity>) workTicketFireService.findByCondition(conditions, page);
			}
		}

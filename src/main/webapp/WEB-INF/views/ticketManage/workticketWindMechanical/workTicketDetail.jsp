	...
	<body>
		...
		<div class="tabbable" style="margin-top: 20px;">
				...
				<input id="b" value="${todoTaskEntity.procInstId}" type="text"/> --%>
					...
					<div id="workPjxxDetail" class="tab-pane fade active in" style="overflow-x:hidden;overflow-y: auto;height: 650px">
			 		</div>
					<!-- 风险卡开始 -->
			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker'], function(A){
					...
					A.loadPage({
								...
								url: format_url("/workTicketWindMechanical/workPjxxDetail/"+workTicketId)
					});
					$('#myTab a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
 						//表单详细
						if($(e.target).attr('href') == "#workPjxxDetail"){
							 print="zjkFengLiJiXie.rptdesign";
							A.loadPage({
								render: "#workPjxxDetail",
								url: format_url("/workTicketWindMechanical/workPjxxDetail/"+workTicketId)
							});
						...
						if($(e.target).attr('href') == "#workCardDetail"){
							...
							A.loadPage({
								...
								url: format_url("/workTicketWindMechanical/workCardDetail/"+workTicketId)
							});
						}
 						//3
						if($(e.target).attr('href') == "#sqsyDetail"){
							...
							A.loadPage({
								render: "#sqsyDetail",
								url: format_url("/workTicketWindMechanical/sqsyDetail/"+workTicketId)
							});
						}
 						//4
						if($(e.target).attr('href') == "#syhfDetail"){
							...
							A.loadPage({
								render: "#syhfDetail",
								url: format_url("/workTicketWindMechanical/syhfDetail/"+workTicketId)
							});
						}
						//流程图

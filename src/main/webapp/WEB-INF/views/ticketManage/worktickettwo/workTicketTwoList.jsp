			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
                       <h5 class="table-title header smaller blue" >电气第二种工作票列表</h5>
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker','selectbox'], function(A){
					...
					var exportExcel="";
					 workTicketDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/workTicketTwo/searchData"),
							...
							columns: [
							         ...
							         /*  {data: "isSetName",name:"isSet",width: "5%",orderable: true}, */
							          ...
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/workTicketTwo/exportExcel"),JSON.stringify(conditions));
											 }
											 exportExcel="";
										 },
							toolbars: [{
											...
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														...
														url : format_url("/workTicketTwo/getAdd")
													});
            									} else{
...
// 										});
								...
								events:{
									click: function(event){
										...
										var url = format_url('/workTicketTwo/bulkDelete/'+ids);
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketTwo/updateValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													A.loadPage({
														render : '#page-container',
														url:format_url("/workTicketTwo/getEdit/"+ id),
													});
            									} else{
            										alert(result.errorMsg);
            									}
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketTwo/deleteValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													var url =format_url('/workTicketTwo/'+ id);
													A.confirm('您确认删除么？',function(){
														$.ajax({
															url : url,
			        										contentType : 'application/json',
								...
								events:{
									click: function(event, nRow, nData){
										...
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketTwo/tijiaoValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
											success: function(result){
												if(result.result == 'success'){
													workticketDialog = new A.dialog({
														title:"提交确认",
														url:format_url("/workTicketTwo/sureSubmit?id="+ id),
														height:450,
														width:500
													}).render();
            									} else{
						...
						/* {
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										$.ajax({
											url: format_url('/workTicketTwo/setValidate/'+id),
											contentType : 'application/json',
											dataType : 'JSON',
											type: 'POST',
											data : JSON.stringify(nData),
						...
						},  */
						{
								...
								events:{
									click: function(event, nRow, nData){
										...
										A.loadPage({
											render : '#page-container',
											url:format_url("/workTicketTwo/detail/"+ id),
										});
									}
								}
						}
						,{
							...
							events:{
								click: function(event, nRow, nData){
									var id = nData.id;
									$.ajax({
										url: format_url('/workTicketTwo/invalidValidate/'+id),
										contentType : 'application/json',
										dataType : 'JSON',
										type: 'POST',
										data : JSON.stringify(nData),
										success: function(result){
											if(result.result == 'success'){
												workticketDialog = new A.dialog({
													title:"鉴定",
													url:format_url("/workTicketTwo/invalid?id="+ id),
													height:350,
													width:600
												}).render();
        									} else{
						...
						events:{
							click: function(event, nRow, nData){
								birtPrint("workTicketTwo.rptdesign",nData.id);
							}
						}
					},{//zzq修改第二次需求
						id: "printAqcs",
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				...
				var url =format_url("/workElectricTwo/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

...
// 					approvetimeDatePicker.setValue('<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${workFireEntity.approveStarttime}" type="date"/>');
					...
					$("#agreeXk").on("click", function(){
						...
						url = format_url("/workFireTwo/agreeXk?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormXk").submit();
    				});
					...
					$("#disagreeXk").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/workFireTwo/disagreeXk?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormXk").submit();
    				});
				});
			});

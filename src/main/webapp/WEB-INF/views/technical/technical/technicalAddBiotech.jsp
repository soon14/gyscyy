			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeJxzg").on("click", function(){
						...
						url = format_url("/technical/agreeBiotech?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormJxzg").submit();
    				});
					$("#disAgreeJxzg").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/technical/disAgreeBiotech?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
				});
			});

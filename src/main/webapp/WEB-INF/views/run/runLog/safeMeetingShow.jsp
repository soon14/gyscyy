	...
	<div class="breadcrumbs ace-save-state" id="breadcrumbs">
		...
		<div class="col-md-12" >
			<form class="form-horizontal" role="form"  style="margin-right:100px;" id="editForm">
			    	<input type="hidden" id="id" name="id" value="${safeMeetingEntity.id}" />
			     <input class="col-md-12" id="rlId" name="rlId" type="hidden" value="${safeMeetingEntity.rlId}">
			    	<input type="hidden" id="checkState" name="checkState" value="${safeMeetingEntity.checkState}" />
			    	<input type="hidden" id="meetingFlag" name="meetingFlag" value="${safeMeetingEntity.meetingFlag}" />
			     <div class="form-group">									
				    <label class="col-md-2 control-label no-padding-right">
				    	...
				    	<div class="col-md-4">
									...
									value='<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${safeMeetingEntity.workTime}" type="date"/>'>
								</div>
                      <label class="col-md-2 control-label no-padding-right">
						负责人
				    </label>
				    <div class="col-md-4">
                	<input name="fZR" type="text" id="fZR" readonly="readonly" value="${safeMeetingEntity.fZR }"/>			                  					                   
                    </div>
               </div>	
                <div class="form-group">									
				    <label class="col-md-2 control-label no-padding-right">
				    ...
				    <div class="col-md-4">
                	<input name="checkState" type="checkbox" disabled="disabled" id="checkStateid"/>			                  					                   
                    </div>
               </div>
                <div class="form-group">
					<label class="col-md-2 control-label no-padding-right">
					...
					<div class="col-md-10">
				     <textarea id="meetingContent" placeholder="请输入工作内容" readonly="readonly" name="meetingContent"  class="col-md-12" style="height:150px; resize:none;">${safeMeetingEntity.meetingContent}</textarea>				    					
                	</div>							   
               </div>
...
// 					plandateStartDatePicker.setValue('<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${dataMap.workTime}" type="both"/>');
					...
					$('#editForm').validate({
						...
						submitHandler: function (form) {
							//添加按钮
							var url = format_url("/safeMeeting/update/"+id);
					...
					$('#button').on('click',function(){
						A.loadPage({
							...
							url : format_url("/safeMeeting/list")
						});
					});

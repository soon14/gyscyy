			...
			<div class="col-lg-12 col-md-12 col-sm-12 search-content  padding-zero">
				...
				<div class="clearfix">
	             	<div class="form-group col-lg-3 col-md-3 col-sm-6 col-xs-12 padding-zero">
	                    <label class="searchLabel" for="searchunitNameId">填报单位</label>：
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >事件通报</h5>
						<table id="eventNotification_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var eventNotificationDatatables = new A.datatables({
						render: '#eventNotification_table',
						options: {
					        "ajax": {
					            "url": format_url("/eventNotification/search"),
							...
							columns: [{data:"id", visible:false,orderable:false}, 
							          ...
							          {data: "fillTime",width: "auto",orderable: true}, 
							          {data: "eventName",width: "auto",orderable: true}, 
							          {data: "statusName",name:"status",width: "auto",orderable: true}, 
							          ],
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/eventNotification/exportExcel"),JSON.stringify(dd));
											 }
											 exportExcel="";
										 },
							toolbars: [{
            						...
            						click:function(event){
        								...
        								if(unitId=="112"||unitId=="113"||unitId=="114"||unitId=="115"||unitId=="116"||unitId=="117"||unitId=="138"||unitId=="141"||unitId=="144"){
        									listFormDialog = new A.dialog({
                        						...
                        						height: '471',
                        						title: "事件通报增加",
                        						url:format_url('/eventNotification/getAdd'),
                        						closed: function(){
                        							eventNotificationDatatables.draw(false);
                        						}	
                        					}).render()
        								}else{
								...
								events:{
									click: function(event){
										var data = eventNotificationDatatables.getSelectRowDatas();
										var ids = [];
										var userIds = [];
										var status = [];
										...
										var url = format_url('/eventNotification/bulkDelete/');
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height: '471',
											title: "事件通报编辑",
											url:format_url('/eventNotification/getEdit/' + id),
											closed: function(){
												eventNotificationDatatables.draw(false);
											}
										}).render();
							...
							events:{
								click: function(event, nRow, nData){
									...
									summaryDialog = new A.dialog({
                						...
                						height:460 ,
                						title: "事件通报查看",
                						url:format_url("/eventNotification/detail/"+ id),
                						closed: function(){
                							eventNotificationDatatables.draw(false);
                						}	
                					}).render()
						...
						events:{
							click: function(event, nRow, nData){
								var id = nData.id;
											summaryDialog = new A.dialog({
												title:"提交确认",
												url:format_url("/eventNotification/sureSubmit?id="+ id),
												height:400,
												width:500
											}).render();
//     								
					...
					$('#btnSearch').on('click',function(){
						...
						eventNotificationDatatables.draw();
					});
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var url =format_url("/eventNotification/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,
					success: function(result){
						if(result.result=="success"){
							...
							A.loadPage({
								...
								url:format_url("/eventNotification/index"),
							});
						}else{

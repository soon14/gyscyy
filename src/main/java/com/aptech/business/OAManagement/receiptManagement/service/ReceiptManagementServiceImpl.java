...
import com.aptech.business.OAManagement.receiptManagement.dao.ReceiptManagementDao;
...
@Transactional
public class ReceiptManagementServiceImpl extends AbstractBaseEntityOperation<ReceiptManagementEntity> implements ReceiptManagementService {

	@Autowired
	private ReceiptManagementDao receiptManagementDao;
	...
	@Override
	public IBaseEntityOperation<ReceiptManagementEntity> getDao() {
		return receiptManagementDao;
	}
	...
	public List<Map<String, String>> getReceiptNumberInfo() {
		...
		List<Map<String, String>> tmp = this.findByCondition("findReceptNumberInfo", conditions, null);
		if (tmp != null) {
			return tmp;
		}
	...
	public void submit(String id, List<SysUserEntity> selectUser) {
		...
		//准备启动流程
		String key=ProcessMarkEnum.RECEIPT_MANAGEMENT_KEY.getName();	
		...
		Map<String, Object> vars=new HashMap<String,Object>();
  		...
  		ReceiptManagementEntity receiptManagementEntity = this.findById(Long.valueOf(id));
  		receiptManagementEntity.setApprovalStatus(ReceiptApprovalStatusEnum.REVIEW.getCode()); 
		
		super.updateEntity(receiptManagementEntity);
	...
	public List<SysUserEntity> getLeaderPerson4ReceivingHandle(String taskId) {
		String [] activityIds = {"sid-898e90d7-e95f-4f8a-a894-1073527fac79"};
		List<Condition> conditions = new ArrayList<Condition>();
...
//		获得职务下的用户和组织关系表的Id
	 ...
	 * 查找当前部门的子部门
	 */
	private List<SysUnitEntity> findSubUnit(SysUnitEntity sysUnitEntity,
			List<SysUnitEntity> unitsList) {
		List<SysUnitEntity> sysunits = new ArrayList<SysUnitEntity>();
		for (SysUnitEntity sysUnit : unitsList) {

			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeZGZR").on("click", function(){
						...
						url = format_url("/eventAnalysis/agreeZGZR?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormJxzg").submit();
    				});
					$("#disAgreeZGZR").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/eventAnalysis/disAgreeZGZR?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
				});
			});

				...
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreePz").on("click", function(){
						...
						$('#addFormPz').validate({
							...
							submitHandler: function (form) {
								...
								var url = format_url("/workLine/agreePz?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
					...
					$("#disAgreePz").on("click", function(){
						...
						$('#addFormPz').validate({
							...
							rules:  {},
							submitHandler: function (form) {
								var url = format_url("/workLine/disAgreePz?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
								//serializeObject()用于Jquery将form转换成用于ajax参数的Javascript Object
								var obj = $("#addFormPz").serializeObject();
								$.ajax({
									url : url,

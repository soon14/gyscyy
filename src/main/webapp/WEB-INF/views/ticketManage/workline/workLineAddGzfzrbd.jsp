							...
							<div class="form-group">
									...
									<div class="col-md-4">
											<%-- <c:if test="${workElectricEntity.changeOldPicName==null||workElectricEntity.changeOldPicName==''}"> --%>
												<input class="col-md-12" id="changeOldPicId" name="changeOldPicId" type="hidden" readonly="readonly" maxlength="64" value="${workTicketLineEntity.guarderId}">
											<input class="col-md-12" id="changeOldPicName" name="changeOldPicName" type="text" readonly="readonly" maxlength="64" value="${workTicketLineEntity.guarderName}">
											<%-- </c:if>
											<c:if test="${workElectricEntity.changeOldPicName!=null&&workElectricEntity.changeOldPicName!=''}">
		...
		<script type="text/javascript">
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					changeSignerDateDatePicker.setValue('<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${workLineEntity.changeSignerDate}" type="date"/>');
					//日期组件
					...
					$("#sureGzfzrbg").on("click", function(){
						...
						url = format_url("/workLine/sureGzfzrbg?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);

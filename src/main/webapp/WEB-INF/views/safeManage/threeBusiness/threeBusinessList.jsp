			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >三项业务</h5>
						<table id="threeBusiness_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var exportExcel="";
					var threeBusinessDatatables = new A.datatables({
						render: '#threeBusiness_table',
						options: {
					        "ajax": {
					            "url": format_url("/threeBusiness/search"),
							...
							columns: [
							          ...
							          {data: "yearNumString",name:"yearNum",width: "15%",orderable: true}, 
							          {data: "businessDateString",name:"businessDate",width: "15%",orderable: true},
							          {data: "userName",name:"createUserId",width: "13%",orderable: true},
							          ],
							          
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/threeBusiness/exportExcel"),JSON.stringify(conditions));
											 }
											 exportExcel="";
										 },
							...
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                						listFormDialog = new A.dialog({
                    						...
                    						height: 500,
                    						title: "三项业务增加",
                    						url:format_url('/threeBusiness/getAdd'),
                    						closed: function(){
                    							threeBusinessDatatables.draw(false);
                    						}	
                    					}).render()
            						}
        						}
								...
								events:{
									click: function(event){
										var data = threeBusinessDatatables.getSelectRowDatas();
										var ids = [];
										var userIds = [];
										...
										var url = format_url('/threeBusiness/allDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height: 500,
											title: "三项业务修改",
											url:format_url('/threeBusiness/getEdit/' + id),
											closed: function(){
												threeBusinessDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/threeBusiness/deleteOne/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:500 ,
										title: "三项业务查看",
										url:format_url('/threeBusiness/getDetail/' + id),
										closed: function(){
											threeBusinessDatatables.draw(false);
										}
									}).render();

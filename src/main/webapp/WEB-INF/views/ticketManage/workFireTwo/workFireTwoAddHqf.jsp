...
<html lang="zh">
	...
	<body>
		<div class="col-md-12" >
			<form class="form-horizontal" role="form" style="margin-right:100px;" id="addFormHQf">
			<div class="form-group">
					...
					<div class="col-md-4">
							...
							<input class="col-md-12" id="meetSignName" name="meetSignName" type="text" readonly="readonly" maxlength="64" value="${userEntity.name}">
					</div>
				<label class="col-md-2 control-label no-padding-right">签发日期</label>
				<div class="col-md-4">
						<input class="col-md-12" id="meetSignDate" name="meetSignDate" type="text" readonly="readonly" maxlength="64" value='<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${dateHQf}" type="date"/>'>
				</div>
			</div>
			<div class="form-group">
    		...
    		<div style="margin-right:100px;">
    			<button id="disAgreeHQf" class="btn btn-xs btn-danger pull-right"  style="margin-right:15px;">
    				<i class="glyphicon glyphicon-remove-circle"></i>
    				驳回
    			</button>
    			<button id="agreeHQf" class="btn btn-xs btn-success pull-right"  style="margin-right:15px;">
			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$('#addFormHQf').validate({
						debug:true,
						rules:  {
							meetSignName:{required:true,maxlength:64},
							meetSignDate:{required:true,maxlength:64},
						...
						submitHandler: function (form) {
							...
							var obj = $("#addFormHQf").serializeObject();
							$.ajax({
								url : url,
					...
					$("#agreeHQf").on("click", function(){
						...
						url = format_url("/workFireTwo/agreeHQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormHQf").submit();
    				});
					$("#disAgreeHQf").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/workFireTwo/disAgreeHQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormHQf").submit();
					});
				});
			});

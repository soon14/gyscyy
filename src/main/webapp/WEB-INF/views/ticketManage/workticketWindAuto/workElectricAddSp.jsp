		...
		<div class="col-md-12" >
			...
			<div class="form-group">
					...
					<div class="col-md-4">
							...
							<input class="col-md-12" id="ondutyName" name="ondutyName" type="text" readonly="readonly" maxlength="64" value="${userEntity.name}">
					</div>
				<label class="col-md-2 control-label no-padding-right">收票日期</label>
				<div class="col-md-4">
						<input class="col-md-12" id="getticketTime" name="getticketTime" type="text" readonly="readonly" maxlength="64" value='<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${dateQf}" type="date"/>'>
				</div>
			</div>
			<div class="form-group">
		...
		<script type="text/javascript">
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$('#addFormSp').validate({
						debug:true,
						rules:  {approveIdea:{required:true,maxlength:64},
							},
					...
					$("#agreeSp").on("click", function(){
						...
						url = format_url("/workTicketWindAuto/agreeSp?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormSp").submit();
    				});
					...
					$("#disAgreeSp").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/workTicketWindAuto/disAgreeSp?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormSp").submit();
					});
				});
			});

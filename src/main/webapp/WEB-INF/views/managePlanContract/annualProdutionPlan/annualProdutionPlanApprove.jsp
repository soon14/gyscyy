		...
		<script type="text/javascript">
		 ...
		 var scienceTechnologyPlanDatatables;
				...
				seajs.use(['combobox','combotree','my97datepicker','uploaddropzone'], function(A){
					...
					$('#technicalBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "生产技术处审核",
    						url:format_url("/annualProdutionPlan/getAddSCJSC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#planBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处审核",
    						url:format_url("/annualProdutionPlan/getAddJHJYC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#manageBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "单位领导审核",
    						url:format_url("/annualProdutionPlan/getAddDWLDTL?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#excuteBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处下文执行",
    						url:format_url("/annualProdutionPlan/getAddJHJYCXW?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#zfBtn').on('click',function(){
						...
						obj.id=${entity.id};
						$.ajax({
							url : format_url("/annualProdutionPlan/disAgreeZf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+""),
							contentType : 'application/json',
							dataType : 'JSON',
							data : JSON.stringify(obj),
							cache: false,
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var taskId=$("#taskId").val();
				var procInstId=$("#procInstId").val();
				var url =format_url("/annualProdutionPlan/againSubmit?workId="+id+"&selectUser="+selectUser+"&taskId="+taskId+"&procInstId="+procInstId);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

...
<!-- 		 			</c:if> -->
				...
				<input id="b" value="${todoTaskEntity.procInstId}" type="text"/> --%>
					...
					<input id="workTicketId" value="${workTicketTwoId}" type="hidden"/>
			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker'], function(A){
					...
					A.loadPage({
								...
								url: format_url("/workTicketTwo/workPjxxDetail/"+workTicketId)
					});
					$('#myTab a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
						...
						if($(e.target).attr('href') == "#workCardDetail"){
							A.loadPage({
								...
								url: format_url("/workTicket/workCardDetail/"+workTicketId)
							});
						}
 						//3
						if($(e.target).attr('href') == "#sqsyDetail"){
							A.loadPage({
								render: "#sqsyDetail",
								url: format_url("/workElectric/sqsyDetail/"+workTicketId)
							});
						}
 						//4
						if($(e.target).attr('href') == "#syhfDetail"){
							A.loadPage({
								render: "#syhfDetail",
								url: format_url("/workElectric/syhfDetail/"+workTicketId)
							});
						}
						//流程图

				...
				<div class="form-inline text-left " role="form">
				 <div class="clearfix">
                        ...
                        <div class="form-group col-lg-6 col-md-6 col-sm-6 col-xs-12 padding-zero">
                   	         <label class="searchLabel">排查时间</label>：
                            <div class="form-group dateInputOther padding-zero text-left">
                                <div id="searchCheckDateStartDiv"  style="border:none; padding:0px;"></div>
                            </div>
                            <div class="toLabel" for="searchStartTimeRightDiv" >~</div>
                            <div class="form-group dateInputOther padding-zero text-left">
                                <div id="searchCheckDateEndDiv"   style="border:none; padding:0px;"></div>
                            </div>
                        </div>
...
<!--                           <div class="form-group col-lg-3 col-md-3 col-sm-6 col-xs-12 padding-zero" > -->
<!--                             <label class="searchLabel" for="searchFileName">相关资料</label>： -->
<!--                              <input id="searchFileName" class="input-width text-left"  placeholder="请输入相关资料" type="text"></input> -->
<!--                         </div> -->
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var searchRequestDateStartDiv = new A.my97datepicker({
						id: 'searchCheckDateStartDivId',
						name:'checkDate',
						render:'#searchCheckDateStartDiv',
						options : {
								isShowWeek : false,
								skin : 'ext',
								maxDate: "#F{$dp.$D(\\'searchCheckDateEndDivId\\')}",
								minDate: "",
								dateFmt: "yyyy-MM-dd"
						}
					}).render();
					...
					var searchRequestDateStartDiv = new A.my97datepicker({
						id: 'searchCheckDateEndDivId',
						name:'checkDate',
						render:'#searchCheckDateEndDiv',
						options : {
								isShowWeek : false,
								skin : 'ext',
								maxDate: "",
								minDate: "#F{$dp.$D(\\'searchCheckDateStartDivId\\')}",
								dateFmt: "yyyy-MM-dd"
						}
					}).render();
					...
					var hiddenTroubleDatatables = new A.datatables({
						...
						options: {
							...
							columns: [
							          ...
							          {data: "rectificationTime",width: "auto",orderable: true},
							...
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                						listFormDialog = new A.dialog({
                    						...
                    						height:550 ,
                    						title: "${targetType}增加",
                    						url:format_url('/hiddenTrouble/getAdd'),
                    						closed: function(){
								...
								events:{
									click: function(event){
										...
										var url = format_url('/hiddenTrouble/allDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height:550 ,
											title: "${targetType}修改",
											url:format_url('/hiddenTrouble/getEdit/' + id),
											closed: function(){
												hiddenTroubleDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/hiddenTrouble/deleteOne/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:550 ,
										title: "${targetType}查看",
										url:format_url('/hiddenTrouble/getDetail/' + id),
										closed: function(){
											hiddenTroubleDatatables.draw(false);
										}
									}).render();
					...
					$('#btnSearch').on('click',function(){
						...
						if($("#searchCheckDateStartDivId").val()!=""){
							conditions.push({
								field: 'checkDate',
								fieldType:'DATE',
								matchType:'GE',
								value:$("#searchCheckDateStartDivId").val()+" 00:00:00"
							});
						}
						if($("#searchCheckDateEndDivId").val()!=""){
							conditions.push({
								field: 'checkDate',
								fieldType:'DATE',
								matchType:'LE',
								value:$("#searchCheckDateEndDivId").val()+" 23:59:59"
							});
						}
...
// 						}
					...
					$('#btnReset').on('click',function(){
						...
						$("#searchCheckDateStartDivId").val("");
						$("#searchCheckDateStartDiv").val("");
						$("#searchCheckDateEndDivId").val("");
						$("#searchCheckDateEndDiv").val("");
						$("#searchFileName").val('');
					});
				});
			});

...
// 			var deleteInfo = {};
			jQuery(function($) {
				...
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					//取消勾选时删除数据
					function deleteUnCheckDate(deleteInfo){
						var params = {};
						params.deleteArray = deleteInfo;
						params.overhaulRecordId = overhaulRecordId;
// 						A.confirm('您确认删除取消勾选的数据吗？',function(){
							$.ajax({
								url: format_url("/overhaulWorkTask/deleteUnCheckDate"),
								contentType: "application/json",
								dataType: 'JSON',
								cache: false,
								type: 'POST',
								...
								success: function(result){
									...
									for(var i=0;i<equipArr.length;i++){
										for(var j=0;j<deleteArray.length;j++){
											//取到删除的物资明细id
				                            var materialId = deleteArray[j].data.id;
											var name=deleteArray[j].data.name;
											//循环数组中的每一个物资明细id
				                            var materialResultId = equipArr[i].code;
											var nRow = deleteArray[j].row;
											//如果两个id相等，在数组中去掉这个id

...
<%-- 				<input id="unitId" name="unitId" value="${overhaulOrganizationId}" type="hidden"/> --%>
				<input id="dutyPerson" name="dutyPerson" value="${recordEntity.dutyUserId}" type="hidden"/>
...
<!--                     </label> -->
<!-- 					<div class="col-md-4"> -->
<!-- 				    	<select class="col-md-12 chosen-select" id="dutyPerson" name="dutyPerson" disabled="disabled"></select> -->
<!-- 				    </div> -->
                </div>
                 <div class="form-group">
                 	<label class="col-md-2 control-label no-padding-right">
...
// 		    var overhaulOrganizationId = ${overhaulOrganizationId};
			...
			var overhaulWorkTaskDatatables;
...
// 工作任务 table展示---------------------------------start-------------------------------------------------------------------------------- 
					 ...
					 overhaulWorkTaskDatatables = new A.datatables({
        						...
        						events:{
            						click:function(event){
            							var equipDialog = new A.dialog({
            									...
            									url:format_url("/overhaulWorkTask/selectEquipLedger"),
            									closed: function(equipInfoArray){
           										var params = {};
           										params.equipInfo = equipInfoArray;
           										var length  = equipInfoArray.length;
            									for(var i=0;i<length;i++){
	            										var equipObj = {};
	            										//同步设备数组
	            										equipids = [];
	            										for(var j=0; j<equipArr.length;j++){
	            						                	   equipids.push(equipArr[j].code);
	            						                }
												...
												$.ajax({
													...
													success: function(result){
														...
														for(var j=0;j<userUnitRels.length;j++){
								                            ...
								                            var materialId = nData.equipId;
															//循环数组中的每一个物资明细id
								                            var equipId = userUnitRels[j].id;
															//如果两个id相等，在数组中去掉这个id
								                            if(materialId == equipId){
							...
							btns:[
								{
									...
									events:{
										"click": function(event, nRow, nData){
											...
											A.confirm('您确认删除么？',function(){
													$.ajax({
														url: format_url("/overhaulSafe/"+id),
														contentType: "application/json",
														dataType: 'JSON',
														type: 'DELETE',
														success: function(result){
							...
							$.ajax({
								...
								success: function(result){
									 if(result.result == "success"){
										 ...
										 A.loadPage({
												render : '#page-container',
// 												url : format_url("/overhaulRecord/getDataList/"+ overhaulOrganizationId)
												url : format_url("/overhaulRecord/getDataList")
										 });
									}else{

			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >安全生产标准化建设</h5>
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','my97datepicker'], function(A){
					...
					var trainPlanDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/safeStandard/search"),
							...
							columns: [
							          ...
							          {data: "standardName",width: "20%",orderable: true}, 
// 							          {data: "standardStyleName",name:"category",width: "15%",orderable: true}, 
							          {data: "time",width: "20%",orderable: true}, 
							          {data: "yearNum",width: "20%",orderable: true}, 
							          {data: "unitName",width: "20%",orderable: true}, 
							          {data: "userName",name:"createPeopleId",width: "20%",orderable: true}
// 							          {data: "fileName",width: "20%",orderable: true}
							          ],
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/safeStandard/exportExcel"),JSON.stringify(conditions));
											 }
											 exportExcel="";
										 },
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                    						listFormDialog = new A.dialog({
                        						...
                        						height:450 ,
                        						title: "安全生成标准化建设增加",
                        						url:format_url('/safeStandard/getAdd'),
                        						closed: function(){
								...
								events:{
									click: function(event){
										...
										var url = format_url('/safeStandard/allDelete/');
							...
							btns: [{
								...
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:450 ,
										title: "安全生成标准化建设修改",
										url:format_url('/safeStandard/getEdit/' + id),
										closed: function(){
											trainPlanDatatables.draw(false);
										}
									}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/safeStandard/deleteOne/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:450 ,
										title: "安全生成标准化建设查看",
										url:format_url('/safeStandard/getshowPage/' + id),
										closed: function(){
											trainPlanDatatables.draw(false);
										}
									}).render();

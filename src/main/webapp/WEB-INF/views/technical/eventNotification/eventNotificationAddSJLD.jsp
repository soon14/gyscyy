			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeSJLD").on("click", function(){
						...
						url = format_url("/eventNotification/agreeSJLD?taskId="+taskId+"&procInstId="+procInstId);
						$("#addFormJxzg").submit();
    				});
					$("#disAgreeSJLD").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/eventNotification/disAgreeSJLD?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
					$("#disAgreeSJLDUP").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/eventNotification/disAgreeSJLDUP?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormJxzg").submit();
					});
				});
			});

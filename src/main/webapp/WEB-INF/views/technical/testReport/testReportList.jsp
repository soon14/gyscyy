			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >试验报告</h5>
						<table id="testReport_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var testReportDatatables = new A.datatables({
						render: '#testReport_table',
						options: {
					        "ajax": {
					            "url": format_url("/testReport/search"),
							...
							columns: [{data:"id", visible:false,orderable:false}, 
							          ...
							          {data: "testMember",width: "auto",orderable: true}, 
							          ...
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/testReport/exportExcel"),JSON.stringify(dd));
											 }
											 exportExcel="";
										 },
							toolbars: [{
            						...
            						click:function(event){
        								...
        								if(unitId=="112"||unitId=="113"||unitId=="114"||unitId=="115"||unitId=="116"||unitId=="117"||unitId=="138"||unitId=="141"){
        									listFormDialog = new A.dialog({
                        						...
                        						height: '471',
                        						title: "试验报告增加",
                        						url:format_url('/testReport/getAdd'),
                        						closed: function(){
                        							testReportDatatables.draw(false);
                        						}	
                        					}).render()
        								}else{
								...
								events:{
									click: function(event){
										var data = testReportDatatables.getSelectRowDatas();
										var ids = [];
										var userIds = [];
										var status = [];
										...
										var url = format_url('/testReport/bulkDelete/');
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height: '471',
											title: "试验报告编辑",
											url:format_url('/testReport/getEdit/' + id),
											closed: function(){
												testReportDatatables.draw(false);
											}
										}).render();
							...
							events:{
								click: function(event, nRow, nData){
									...
									summaryDialog = new A.dialog({
                						...
                						height:460 ,
                						title: "试验报告查看",
                						url:format_url("/testReport/detail/"+ id),
                						closed: function(){
                							testReportDatatables.draw(false);
                						}	
                					}).render()
						...
						events:{
							click: function(event, nRow, nData){
								var id = nData.id;
											summaryDialog = new A.dialog({
												title:"提交确认",
												url:format_url("/testReport/sureSubmit?id="+ id),
												height:400,
												width:500
											}).render();
//     								
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var url =format_url("/testReport/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,
					success: function(result){
						if(result.result=="success"){
							...
							A.loadPage({
								...
								url:format_url("/testReport/index"),
							});
						}else{

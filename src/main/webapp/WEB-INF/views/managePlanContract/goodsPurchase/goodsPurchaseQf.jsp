			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeQf").on("click", function(){
						...
						url = format_url("/goodsPurchase/agreeQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormQf").submit();
    				});
					$("#disAgreeQf").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/goodsPurchase/disAgreeQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+"");
						$("#addFormQf").submit();
					});
				});
			});

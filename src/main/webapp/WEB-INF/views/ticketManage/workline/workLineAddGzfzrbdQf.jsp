			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$("#agreeGzfzrbgQf").on("click", function(){
						var yuanPic='${workLineEntity.changeOldPicId}';
...
// 						}
						...
						url = format_url("/workLine/agreeGzfzrbgQf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormGzfzrbdQf").submit();
    				});
					$("#disAgreeGzfzrbgQf").on("click", function(){
						var taskId=$("#taskId").val();
						var procInstId=$("#procInstId").val(); 
						url = format_url("/workLine/disAgreeGzfzrbgQf?taskId="+taskId+"&procInstId="+procInstId);
						$("#addFormGzfzrbdQf").submit();
    				});

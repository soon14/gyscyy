	...
	<body>
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active">安全活动</li><%--  ${targetType} --%>
			</ul><!-- /.breadcrumb -->
		</div>
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class="table-title header smaller blue" >安全活动</h5>
						<table id="safeActivity_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
...
// 					}).render();
					...
					var exportExcel="";
					var safeActivityDatatables = new A.datatables({
						render: '#safeActivity_table',
						options: {
					        "ajax": {
					            "url": format_url("/safeActivity/dataList"),
...
// 							          {data: "fileName",width: "19%",orderable: true}
							         ...
							         fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
										 if(exportExcel){
											 exportExcels(format_url("/safeActivity/exportExcel"),JSON.stringify(conditions));
										 }
										 exportExcel="";
									 },
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                						listFormDialog = new A.dialog({
                    						...
                    						height:400 ,
                    						title: "安全活动增加",
                    						url:format_url('/safeActivity/getAdd'),
                    						closed: function(){
                    							safeActivityDatatables.draw(false);
                    						}	
                    					}).render()
            						}
        						}
								...
								events:{
									click: function(event){
										var data = safeActivityDatatables.getSelectRowDatas();
										var ids = [];
										var userIds = [];
										...
										var url = format_url('/safeActivity/bulkDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height:400 ,
											title: "安全活动修改",
											url:format_url('/safeActivity/getEdit/' + id),
											closed: function(){
												safeActivityDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/safeActivity/delete/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:400 ,
										title: "安全活动查看",
										url:format_url('/safeActivity/getDetail/' + id),
										closed: function(){
											safeActivityDatatables.draw(false);
										}
									}).render();

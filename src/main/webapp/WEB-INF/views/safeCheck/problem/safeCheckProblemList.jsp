	...
	<body>
		<div class="breadcrumbs ace-save-state" id="breadcrumbs" style="margin-bottom:0px;">
			<ul class="breadcrumb">
				...
				<li class="active">安全检查问题</li>
				<li class="active">安全检查问题填报</li>
			</ul>
		</div>
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >安全检查问题</h5>
						<table id="safeCheckProblem_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var safeCheckProblemDatatables = new A.datatables({
						render: '#safeCheckProblem_table',
						options: {
							"ajax": {
								"url": format_url("/safeCheckProblem/searchData"),
							...
							toolbars: [{
								...
								events:{
									click:function(event){
										listFormDialog = new A.dialog({
											...
											height: '600',
											title: "安全检查问题增加",
											url:format_url('/safeCheckProblem/getAdd/' + ${unitId}),
											closed: function(){
												safeCheckProblemDatatables.draw(false);
											}	
										}).render()
									}
								}
								...
								events:{
									click: function(event){
										var data = safeCheckProblemDatatables.getSelectRowDatas();
										var ids = [];
										...
										var url = format_url('/safeCheckProblem/bulkDelete/');
								...
								events:{
									click: function(event){
										A.loadPage({
				    						...
				    						url : format_url("/safeCheckUnit/index/")
				    					});
									}
								}
							}],
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height: '600',
											title: "安全检查问题编辑",
											url:format_url('/safeCheckProblem/getEdit/' + id),
											closed: function(){
												safeCheckProblemDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/safeCheckProblem/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
												contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height: '600',
										title: "安全检查问题查看",
										url:format_url('/safeCheckProblem/getView/' + id),
										closed: function(){
											safeCheckProblemDatatables.draw(false);
										}
									}).render();
					...
					$('#btnSearch').on('click',function(){
						...
						safeCheckProblemDatatables.draw();
					});

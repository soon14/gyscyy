	...
	<body>
		<div class="col-md-12" >
			<form class="form-horizontal" role="form" style="margin-right:100px;" id="addFormAQ">
								...
								<input class="col-md-12" id="otherExecutorId" name="otherExecutorId" type="hidden"   value="${workFireEntity.otherExecutorId}">			
			<div class="form-group">
					<label class="col-md-2 control-label no-padding-right">安全总监签名</label>
					<div class="col-md-4">
							<input class="col-md-12" id="safeDirctorName"  type="text" readonly="readonly" maxlength="64" value="${userEntity.name}">
					</div>
					<label class="col-md-2 control-label no-padding-right">日期</label>
					<div class="col-md-4">
							<input class="col-md-12" id="safeDirctorDate" name="safeDirctorDate" type="text" readonly="readonly" maxlength="64" value='<fmt:formatDate  pattern="yyyy-MM-dd HH:mm" value="${dateQf}" type="date"/>'>
					</div>
			</div>
				<div class="form-group">
			...
			jQuery(function($) {
				seajs.use(['combobox','combotree','my97datepicker','duallistbox'], function(A){
					...
					$('#addFormAQ').validate({
						debug:true,
						rules:  {
							safeDirctorName:{required:true,maxlength:64},
							safeDirctorDate:{required:true,maxlength:64},
					...
					$("#agreeAQ").on("click", function(){
						...
						url = format_url("/workFire/agreeAqzj?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+selectUser);
						$("#addFormAQ").submit();

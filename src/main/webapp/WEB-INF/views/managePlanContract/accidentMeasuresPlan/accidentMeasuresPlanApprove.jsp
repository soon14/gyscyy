			...
			<form class="form-horizontal" role="form"  style="margin-right:100px;" id="addForm">
			  ...
			  <div class="form-group" style="margin-top: 30px">
		       ...
		       <div class="form-group">
				    ...
				    <div class="col-md-4">
						<input class="col-md-12" id="uploadPerson" name="uploadPerson" readonly="readonly" type="text" placeholder="" maxlength="20" value="${sysUserEntity.name }">
                    </div>
               </div>
		       <div class="form-group">
				    <label class="col-md-2 control-label no-padding-right">
				    ...
				    <div class="col-md-10">
						<textarea placeholder="请输入备注" readonly="readonly" name="remark" class="col-md-12" style="height:90px; resize:none;">${entity.remark }</textarea>
                    </div>
               </div>
               <div class="form-group">
               <label class="col-md-2 control-label no-padding-right">
		...
		<script type="text/javascript">
		 ...
		 var scienceTechnologyPlanDatatables;
				...
				seajs.use(['combobox','combotree','my97datepicker','uploaddropzone'], function(A){
					...
					$('#addForm').validate({
						...
						submitHandler: function (form) {
							//添加按钮
							var url = format_url("/accidentMeasuresPlan");
					...
					$('#technicalBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "生产技术处审核",
    						url:format_url("/accidentMeasuresPlan/getAddSCJSC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#planBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处审核",
    						url:format_url("/accidentMeasuresPlan/getAddJHJYC?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#manageBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "单位领导审核",
    						url:format_url("/accidentMeasuresPlan/getAddDWLDTL?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#excuteBtn').on('click',function(){
						workSafeOneDialog = new A.dialog({
    						...
    						height: 400,
    						title: "计划经营处下文执行",
    						url:format_url("/accidentMeasuresPlan/getAddJHJYCXW?id="+id+"&taskId="+taskId ),
    						closed: function(){
    						}	
    					}).render();
					});
					...
					$('#zfBtn').on('click',function(){
						...
						obj.id=${entity.id};
						$.ajax({
							url : format_url("/accidentMeasuresPlan/disAgreeZf?taskId="+taskId+"&procInstId="+procInstId+"&selectUser="+""),
							contentType : 'application/json',
							dataType : 'JSON',
							data : JSON.stringify(obj),
							cache: false,
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var taskId=$("#taskId").val();
				var procInstId=$("#procInstId").val();
				var url =format_url("/accidentMeasuresPlan/againSubmit?workId="+id+"&selectUser="+selectUser+"&taskId="+taskId+"&procInstId="+procInstId);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,

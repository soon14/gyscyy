			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-main no-padding">
						<h5 class='table-title header smaller blue' >事件分析</h5>
						<table id="eventAnalysis_table" class="table table-striped table-bordered table-hover" style="width:100%;">
							<thead>
								<tr>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var eventAnalysisDatatables = new A.datatables({
						render: '#eventAnalysis_table',
						options: {
					        "ajax": {
					            "url": format_url("/eventAnalysis/search"),
							...
							columns: [{data:"id", visible:false,orderable:false}, 
							          ...
							          fnPreDrawCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
											 if(exportExcel){
												 exportExcels(format_url("/eventAnalysis/exportExcel"),JSON.stringify(dd));
											 }
											 exportExcel="";
										 },
							toolbars: [{
            						...
            						click:function(event){
        								...
        								if(unitId=="112"||unitId=="113"||unitId=="114"||unitId=="115"||unitId=="116"||unitId=="117"||unitId=="138"||unitId=="141"){
        									listFormDialog = new A.dialog({
                        						...
                        						height: '471',
                        						title: "事件分析增加",
                        						url:format_url('/eventAnalysis/getAdd'),
                        						closed: function(){
                        							eventAnalysisDatatables.draw(false);
                        						}	
                        					}).render()
        								}else{
								...
								events:{
									click: function(event){
										var data = eventAnalysisDatatables.getSelectRowDatas();
										var ids = [];
										var userIds = [];
										var status = [];
										...
										var url = format_url('/eventAnalysis/bulkDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height: '471',
											title: "事件分析编辑",
											url:format_url('/eventAnalysis/getEdit/' + id),
											closed: function(){
												eventAnalysisDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/eventAnalysis/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									summaryDialog = new A.dialog({
                						...
                						height:460 ,
                						title: "事件分析查看",
                						url:format_url("/eventAnalysis/detail/"+ id),
                						closed: function(){
                							eventAnalysisDatatables.draw(false);
                						}	
                					}).render()
						...
						events:{
							click: function(event, nRow, nData){
								var id = nData.id;
											summaryDialog = new A.dialog({
												title:"提交确认",
												url:format_url("/eventAnalysis/sureSubmit?id="+ id),
												height:400,
												width:500
											}).render();
//     								
			...
			function goBackToSubmitPerson(id,selectUser){//回调函数
				var url =format_url("/eventAnalysis/submit?workId="+id+"&selectUser="+selectUser);
				$.ajax({
					contentType: "application/json",
					dataType:"JSON",
					url : url,
					success: function(result){
						if(result.result=="success"){
							...
							A.loadPage({
								...
								url:format_url("/eventAnalysis/index"),
							});
						}else{
